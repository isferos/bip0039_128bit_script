...
seed_str = ["project", "tomato", "office", "rifle", "actress", "enemy", "between", "sport", "only", "alley", "skin", "student"]


def seedToBin132(word_list, seed):
    seed_bin = []
    for i in range(12):
        seed_bin.append(word_list.index(seed[i]))
    return seed_bin

def getByteFromBit(seed, bit):
    a = bit / 11
    b = (bit+8) / 11
    if a == b:
        return (seed[a] >> (11 - ((bit + 8) % 11))) & 0xFF
    else:
        a = (seed[a] << ((bit % 11) - 3))
        b = (seed[b] >> (14 - (bit % 11)))
        return (a | b) & 0xFF

def makeChecksum(seed_bin):
    m = sha256()
    for i in range(16):
        m.update(chr(getByteFromBit(seed_bin, 8 * i)))
    d = m.digest()
    return ord(d[0])>>4
    
def isSeedValid(word_list, seed_str):
    seed_bin = seedToBin132(word_list, seed_str)
    expected_checksum = seed_bin[11] & 0x0F
    checksum = makeChecksum(seed_bin)
    return (checksum == expected_checksum)

def getPossibleWord(word_list, seed_str, unsure_word):
    count = 1
    for i in range(len(word_list)):
        seed_str[unsure_word] = word_list[i]
        if(isSeedValid(word_list, seed_str)):
            print "%d: Word was possibly: \"%s\"" % (count, word_list[i])
            count = count + 1

try:
    getPossibleWord(word_list, seed_str, 0)
except:
